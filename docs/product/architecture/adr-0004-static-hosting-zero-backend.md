...
**KPI-2 aggregate-counter instrumentation**, _if and when implemented_, MUST be a single stateless endpoint with the following constraints:
...
- _Rejected because:_ (a) nothing in scope requires server-side logic; (b) introduces potential privacy hazards (accidental logging, third-party telemetry from the provider's SDK) that a static-only deploy sidesteps entirely; (c) adds cost, ops complexity, and attack surface for no functional gain.

**Dedicated Node server** (Express / Fastify on a VPS).

- _Rejected because:_ radically overkill. All the downsides of the serverless option plus manual patching and uptime responsibility. No benefits.

**Peer-to-peer / offline-first with service worker only** (no host network at all after first load).

- _Not rejected — partially adopted._ Post-first-load the SPA is fully offline-capable by design (no runtime fetches). A service worker for full PWA installability is out of scope for MVP but compatible with this ADR; DEVOPS wave can add one without architectural change.
